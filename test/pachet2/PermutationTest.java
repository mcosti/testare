...
public class PermutationTest {
    ...
    public void testExceptionIsThrownEmptyString() {
        ...
        tester.permute(emptyString, 1);
    }

    @Test(expected = IllegalArgumentException.class)
    public void testExceptionIsThrownLongString() {
        Permutation tester = new Permutation();
        tester.permute(lorem101, 1);
    }

    @Test(expected = IllegalArgumentException.class)
    public void testExceptionIsThrownTimesTooLow() {
        Permutation tester = new Permutation();
        tester.permute(easyString, 0);
    }

    @Test(expected = IllegalArgumentException.class)
    public void testExceptionIsThrownTimesTooHigh() {
        Permutation tester = new Permutation();
        tester.permute(easyString, 101);
    }

    @Test
    public void testPermuteOne() {
        Permutation tester = new Permutation();
        assertEquals("bcdefa", tester.permute(easyString, 1));
    }

    @Test
    public void testPermuteTwo() {
        Permutation tester = new Permutation();
        assertEquals("efabcd", tester.permute(easyString, 2));
    }

    @Test
    public void testLorem99Times() {
        Permutation tester = new Permutation();
        assertEquals("mLorem ipsum dolor sit amet, consectetuer adipiscing elit. Aenean commodo ligula eget dolor. Aenean ", tester.permute(lorem100, 99));
    }

    @Test
    public void testLoremOdd() {
        Permutation tester = new Permutation();
        assertEquals(lorem99, tester.permute(lorem99, lorem99.length()));
    }

    @Test
    public void testLoremEven() {
        Permutation tester = new Permutation();
        assertEquals(lorem100, tester.permute(lorem100, lorem100.length()));
    }
